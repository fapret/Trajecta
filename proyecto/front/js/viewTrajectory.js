...
const url = `${baseUrl}/curricula_microservice/Faculty/Carrera/Plan/eval`;
...
let apiUrl = `http://${baseUrl}/curricula_microservice/Faculty/ucs?faculty=${facultyName}&uuid=${workspaceID}&curricularUnit=` + materias[i];
                fetch(apiUrl)
                    .then(response => {
                        if (!response.ok) {
