...
/**
 * Servlet implementation class EstudianteAddDegree
 */
@MultipartConfig
public class EstudianteAddDegree extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public EstudianteAddDegree() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(PlanInscription planInsc : estudiante.getStudentPlanInscription()) {
			if(planInsc.getPlan().getCareer_parent().getName().equals(career) && Integer.toString(planInsc.getPlan().getYear()).equals(plan)) {
				//If a degree already exists, its rewritten
				Degree deg = Estudiantes.EstudiantesFactory.eINSTANCE.createDegree();
				...
				deg.setDate(selectedDate);
				planInsc.setDegree(deg);
